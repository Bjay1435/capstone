module feature_processor
	...
	logic [16:0] addr;
	logic [16:0] dout, doutb, reg_dout;
	logic [16:0] integral_pixel_value;
	assign addr = ({7'd0,col_index}/2) + (({7'd0,row_index}/2)*17'd320);
	assign doutb = dout;
	logic [16:0] reg_addr, reg_addr2, reg_addr3;

	assign integral_pixel_value = reg_dout;
    ...
    assign integral_mem_addr = reg_addr;
	...
	always_ff @(posedge clk, negedge rst) begin
		if (~rst) begin
		  ...
		  if (reg_wfr[16] == 1) begin
		      failed <= ((~reg_wfr)+17'd1) < features[current_feature2].threshold || ((~reg_wfr)+17'd1) > (features[current_feature2].threshold+{5'h0, SW[6:0], 5'h0});//17'h00600);
		                //((~reg_wfr)+17'd1) < (features[current_feature2].threshold-17'h100) ||
		                //((~reg_wfr)+17'd1) > (features[current_feature2].threshold+17'h100);
		      ...
		      failed <= reg_wfr < features[current_feature2].threshold || reg_wfr > (features[current_feature2].threshold+{5'h0, SW[6:0], 5'h0});//17'h00600);
		                //reg_wfr < (features[current_feature2].threshold-17'h100) ||
		                //reg_wfr > (features[current_feature2].threshold+17'h100);
